...
from utils import plot_confusion_matrix, plot_pr, plot_roc ,GENRE_LIST, GENRE_DIR, TEST_DIR
...
def train_model(clf_factory, X, Y, name, plot=False):
    ...
    pr_scores, precisions, recalls, thresholds = list(defaultdict(list)), list(defaultdict(list)), list(defaultdict(list)), list(defaultdict(list))

    roc_scores, tprs, fprs = list(defaultdict(list)), list(defaultdict(list)) ,list(defaultdict(list))
    ...
    summary = (np.mean(scores), np.std(scores))   #222pr_scores[label].append(auc(recall, precision))
    print(summary)

    #save the trained model to disk
    joblib.dump(clf, r'C:\Users\Rag9704\Documents\GitHub\Music_Genre_Classification\my_model.pkl')
...
if __name__ == "__main__":
    ...
    train_avg, test_avg, cms = train_model(None, X, y, "Log Reg FFT",plot=False)  #222
